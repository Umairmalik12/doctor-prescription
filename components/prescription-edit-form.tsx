...
import { Save, Printer, Plus, Trash2, List, ArrowLeft } from "lucide-react"
...
export default function PrescriptionEditForm({ prescription: initialPrescription, medicines: initialMedicines, prescriptionId }: PrescriptionEditFormProps) {
  const { toast } = useToast()
  const router = useRouter()

  // Initialize form state with the passed parameters
  ...
  const [savedPrescriptionId, setSavedPrescriptionId] = useState<string | null>(prescriptionId)
  // Hydration-safe date
  ...
  }, [prescription, medicines, prescriptionId, toast])
  ...
  const resetForm = useCallback(() => {
    ...
    setRelationField("")
  }, [initialPrescription, initialMedicines])
      ...
      <div className="max-w-7xl mx-auto mb-4">
        <div className="bg-white rounded-lg shadow-md p-3 sm:p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Edit Prescription - {prescription.patient_name}</h1>
          </div>
